    ...
    return (
        <div className="min-h-screen bg-gray-100 dark:bg-slate-900">
            ...
            <div className="md:pl-20 flex">
                ...
                {/* RIGHT - Sidebar */}
                <div className="w-80 bg-white dark:bg-slate-800 border-l p-6 space-y-6 sticky top-0 h-screen overflow-auto hidden lg:block">
                    <h2 className="text-lg font-semibold">Download Invoice</h2>
                        ...
                        <div className="flex gap-2 flex-wrap">
                            {THEMES.map((t) => (
                                <button key={t.key} onClick={() => handleThemeSelect(t)}
                                    className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${currentThemeKey === t.key ? "ring-2 ring-offset-2 ring-gray-400 scale-110" : "hover:scale-105"}`}
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            {TEMPLATES.map((t) => (
                                <button key={t.id} onClick={() => setTemplate(t.id)}
                                    ...
                                    {/* Premium Badge */}
                                    {t.premium && (
                                        <div className="absolute top-1 left-1 w-5 h-5 bg-gradient-to-r from-yellow-400 to-amber-500 rounded-full flex items-center justify-center shadow-sm">
                                            <Crown className="w-3 h-3 text-white" />
                                        </div>
                                    )}
                                    <div className="h-12 bg-gray-100 dark:bg-gray-700 rounded mb-2 flex items-center justify-center">
                                        <FileText className="w-5 h-5 text-gray-400" />
                                    </div>
                                    <div className="flex items-center justify-center gap-1">
                                        <p className="text-xs font-medium capitalize">{t.name}</p>
                                        {t.premium && <span className="text-[10px] text-amber-600 font-bold">PRO</span>}
                                    </div>
                                    {template === t.id && <div className="absolute top-1 right-1 w-5 h-5 bg-indigo-500 rounded-full flex items-center justify-center"><Check className="w-3 h-3 text-white" /></div>}
                                </button>
                            ))}
                        </div>
                    </div>
